...
import "../css/CulturePop.css";
import "../css/Models.css";

// Import all Models components for different screen sizes
import ModelsMobile from "./Models";
import ModelsTablet from "./MediumScreenModels";
import ModelsSmallDesktop from "./LargerScreenModels";
import ModelsDesktop from "./LargestScreenModels";

const montserrat = { fontFamily: "'Montserrat', Arial, Helvetica, sans-serif" };

export default function CultureDesktopPopup({ openCulturePopup, toggleCulturePopup }) {
    ...
    const [modelsAnimate, setModelsAnimate] = useState(false);
    ...
    // Detect screen size for responsive Models section
    useEffect(() => {
        const handleResize = () => {
    ...
    useEffect(() => {
        if (openCulturePopup && menuModels.length > 0 && activeSection === "models") {
            setVisibleCount(0);
    ...
    }, [openCulturePopup, menuModels, activeSection]);

    useEffect(() => {
        if (openCulturePopup) {
            setIsClosing(false);
            setTimeout(() => setAnimate(true), 10);
            setTimeout(() => setContentAnimate(true), 300);
    ...
    useEffect(() => {
        if (activeSection === "models") {
            setTimeout(() => setModelsAnimate(true), 100);
        } else {
            setModelsAnimate(false);
        }
    ...
    // Render the appropriate Models component based on screen size
    const renderResponsiveModels = () => {
        switch (screenSize) {
            case "mobile":
                return <ModelsMobile openModels={activeSection === "models"} />;
            case "tablet":
                return <ModelsTablet openMediumModels={activeSection === "models"} />;
            case "small-desktop":
                return <ModelsSmallDesktop openLargerModels={activeSection === "models"} />;
            default:
                return (
                    <div className="relative flex" style={{ height: "calc(100vh - 160px)" }}>
                        {/* Left panel: car list */}
                        <div
                            className="flex flex-col pr-4 pb-13 mt-15 max-h-[580px] overflow-y-auto overflow-x-hidden"
                            style={{
                                scrollbarWidth: "thin",
                        ...
                        {/* Right panel: detail WITH ANIMATIONS */}
                            ...
                            >
                                <div className="pb-24">
                                    <div className="flex items-start gap-6 w-full">
                                        {/* LEFT column with fadeFromTop animations */}
                                        ...
                                        {/* RIGHT column with slideFromLeft and slideFromRight animations */}
                                        <div
    ...
    // Show popup during closing animation
    if (!openCulturePopup && !animate && !isClosing) return null;

    return (
        <div 
            className={`fixed inset-0 bg-white z-50 overflow-hidden transition-all duration-700 ease-in-out ${
                openCulturePopup && animate 
        ...
        >
            ...
            <style>{`
                ...
                .custom-scrollbar-content::-webkit-scrollbar-corner {
                    background: transparent;
                }
            `}</style>
            ...
            <div className="fixed top-0 left-0 right-0 bg-white z-40">
                <div className="w-full flex items-center justify-between pl-12 pr-19 mt-11 mb-7 h-[60px] bg-white">
                    ...
                    <div className="flex flex-col items-center mt-2 absolute left-1/2 pr-3 -translate-x-1/2">
                        <img src="/images/bentleyFooter.png" alt="Bentley" className="w-[110px] h-auto" />
                    </div>
                    <div className="flex items-center gap-0">
                        <div className="group border border-black px-4 py-2 cursor-pointer hover:bg-gray-300 transition-colors duration-200">
                            <span className="text-[11.5px] tracking-[0.15em] text-black font-medium uppercase">
                                REQUEST TEST DRIVE
                            </span>
                        </div>
                        <div className="group cursor-pointer hover:bg-gray-300 px-4 py-2.5 transition-colors duration-200">
            ...
            <div className="pt-40 h-full custom-scrollbar-content">
                <div className="px-12 relative">

                    {/* SINGLE CLOSE BUTTON - animates from right for all sections */}
                    <div 
                        className={`absolute -top-4 right-17 cursor-pointer hover:bg-gray-300 p-3 text-gray-600 flex items-center gap-2 text-[13px] font-medium tracking-widest z-30 transition-all duration-800 delay-700 ease-out ${
                            contentAnimate && !isClosing
                                ? "opacity-100 translate-x-0" 
                                : "opacity-0 translate-x-20"
                        }`}
                        onClick={() => toggleCulturePopup(false)}
                    >
                    ...
                    {activeSection === "models" && renderResponsiveModels()}

                    {/* CULTURE SECTION - ONLY ON DESKTOP AND LARGER */}
                    {(screenSize === "desktop" || screenSize === "small-desktop") && activeSection === "culture" && (
                        <>
                            <div className={`mt-4 transition-all duration-800 delay-300 ease-out ${
                                cultureAnimate && !isClosing
                                    ? "opacity-100 translate-x-0" 
                                    : "opacity-0 -translate-x-20"
                            }`}>
                    ...
                    {/* LIFESTYLE SECTION - ONLY ON DESKTOP AND LARGER */}
                    {(screenSize === "desktop" || screenSize === "small-desktop") && activeSection === "lifestyle" && (
                    ...
                    {/* YOUR BENTLEY SECTION - ONLY ON DESKTOP AND LARGER */}
                    {(screenSize === "desktop" || screenSize === "small-desktop") && activeSection === "yourBentley" && (
                        <div className="w-full h-full overflow-y-auto">
                            <div className={`mt-4 transition-all duration-800 delay-300 ease-out ${
                    ...
                    {/* ABOUT BENTLEY SECTION - ONLY ON DESKTOP AND LARGER */}
                    {(screenSize === "desktop" || screenSize === "small-desktop") && activeSection === "aboutBentley" && (
                        <div className="w-full h-full overflow-y-auto">
                            <div className={`mt-4 transition-all duration-800 delay-300 ease-out ${
